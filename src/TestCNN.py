...
#MODEL_PATH = os.path.join("saved_models", "model_wt.pth")
...
def load_ckp(checkpoint_fpath, model, optimizer):
    ...
    valid_acc_max = checkpoint['val_acc_max']
...
ckp_path = "./checkpoint/current_checkpoint_test.pt"
model, optimizer, start_epoch, valid_acc_max = load_ckp(ckp_path, model, optimizer)
print("model = ", model)
print("optimizer = ", optimizer)
print("start_epoch = ", start_epoch)
print("valid_acc_max = ", valid_acc_max)
print("valid_acc_max = {:.5f}".format(valid_acc_max))
...
def train(start_epoch, n_epochs, val_acc_max, train_loader, val_loader, criterion, optimizer, model, checkpoint_path, best_model_path):
    ...
    #  make sure to continue from last saved accuracy
    val_acc_max = valid_acc_max * len(val_loader)
...
# lets train again for a few epochs
start = 1
end = 2
trained_model, accuracy_stats, loss_stats = train(start, end, valid_acc_max, train_loader, val_loader, criterion,optimizer, model, "./checkpoint/current_checkpoint_best.pt","./best_model/best_model_best.pt")
...
print('Accuracy of the network on {} test images: {}%'.format(len(test_loader), round(test_acc.item()*100.0/len(test_loader), 2)))

### lets compute confusion matrix [testset]
y_pred_list = []
y_true_list = []
with torch.no_grad():
